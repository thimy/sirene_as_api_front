export default {
  methods: {
    ...
    isIdAssociation: function (input) {
      const idAssociationImport = RegExp(/^(\d{3}[SP]\d{4,10}(.\w{1,3}\d{0,5})?|\d[A-Z0-9]\d[SP]\w(\w-?\w{0,2}\d{0,6})?)$/)
    ...
    // Most optimal way to remove diacritics :
    removeDiacritics: function (str) {
      ...
      for (let i = 0; i < diacriticsRemovalMap.length; i++) {
        let letters = diacriticsRemovalMap[i].letters
